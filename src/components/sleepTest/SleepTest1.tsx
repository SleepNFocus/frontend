                ...
                {isFinished ? (
                    <View style={styles.resultBox}>
                        ...
                        <View style={styles.resultTextBox}>
                            ...
                            <Text style={styles.boldText}> {Math.round(clickTimes.reduce((a, b) => a + b, 0) / MAX_STEP)}ms </Text>
                        </View>
                        <Button title='다음' variant="outline" onPress={()=>{navigation.navigate('SleepTest2Desc')}}/>
                    </View>
                ) : (
                    <View style={styles.testBox}>
                        <Text style={styles.progress}> 진행 : {step + 1}/{MAX_STEP} </Text>
                        <Text style={styles.title}> 초록 불을 잡아라! </Text>
                        <View style={styles.circleWrapper}>
                            {isWaiting ? (
                            <View style={styles.waitBox}>
                                <Text style={styles.plus}> + </Text>
                                <Text style={styles.text}> 자극이 나타날 때까지 기다려주세요 </Text>
                            </View>
                            ) : (
                        ...
                        {clickTimes.length > 0 && (
                            ...
                            <Text> 평균 반응속도: {Math.round(clickTimes.reduce((a, b) => a + b, 0) / clickTimes.length)}ms </Text>
                            </View>
                        )}
                    </View>
                )}
