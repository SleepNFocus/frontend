...
export const socialLogin = async (provider: 'kakao' | 'google', access_token: string) => {
  ...
  const response = await apiClient.post('/api/users/social-login/', {
    provider,
    access_token,
  });
...
export const refreshToken = async (refresh: string) => {
  const apiClient = await getApiClient();
  const response = await apiClient.post('/api/users/token/refresh/', { refresh });
  return response.data;
};
...
export const withdrawal = async () => {
  const apiClient = await getApiClient();
  const response = await apiClient.delete('/api/users/withdrawal/');
  return response.data;
};
...
export const saveBasicInfo = async (data: { gender: string; birth_year: number; mbti: string }) => {
  const apiClient = await getApiClient();
  const response = await apiClient.post('/api/users/onboarding/basic/', data);
  return response.data;
};
...
export const saveJobInfo = async (data: { cognitive_type: string; work_time_pattern: string }) => {
  const apiClient = await getApiClient();
  const response = await apiClient.post('/api/users/onboarding/job/', data);
  return response.data;
};
